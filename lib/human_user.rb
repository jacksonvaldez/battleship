class HumanUser
  ...
  def setup_board
    ...
    until loop == false
      ...
      @ships.each{|ship| puts " * #{ship.name}: #{ship.length}".yellow}
      ...
      if choice != 'finish'
        choice = choice.split(',')
        ship_choice = @ships.find{|ship| ship.name == choice[0].to_s.delete(' ') }
        coordinates = choice[1].to_s.split(' ')
        ...
        if ship_choice.class == Ship && @board.valid_placement?(ship_choice, coordinates) && coordinates.length >= 2
          ...
          @ships.reject!{|ship| ship.name == ship_choice.name}
        end
        loop = true
      else
        break
